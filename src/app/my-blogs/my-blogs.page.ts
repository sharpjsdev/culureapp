...
@Component({
  ...
  templateUrl: './my-blogs.page.html',
  styleUrls: ['./my-blogs.page.scss'],
})
...
private customProgressBar(current: number, total: number): string {
	...
	async presentActionSheet(blog,index) {
		if (this.user.user_id == blog.user_id) {
				...
				this.opt = [{
					...
					icon: 'trash',
					handler: () => {
					    this.delete_blog(blog.id, blog.user_id, index);
...
console.log("enter");
    ...
    this.fetch.getMyBlog(start).subscribe(res => {
      const data = res.data;
      if (data.length !== 0) {
        for (const it of data) {
          it.desc = this.trimString(it.body, 60);
          this.blogs.push(it);
        }
        this.start += 5;
      }
	...
	delete_blog(blogid: number, userid: number, index: number) {
    const data = {
      blog_id: blogid,
      user_id: userid,
    };
  ...
  async presentAlertdeletepost(data,index) {
				...
				cssClass: 'my-custom-class',
				header: 'Delete!',
				message: 'Do you really want to <strong>delete</strong> this Blog ?',
				buttons: [{
					text: 'Cancel',
					role: 'cancel',
					cssClass: 'secondary',
					...
					handler: () => {
						...
						self.fetch.deleteBlog(data).subscribe(async (res) => {
            if (res.status === 'success') {
