...
export class EditBlogPage implements OnInit {
  ...
  public blog = JSON.parse(localStorage.getItem('editBlog'));
  ...
  if (this.imageSelectedStatus) {
    ...
    await loading.present();
      ...
      fd.append('user_id', this.blog.user_id);
    ...
    this.fetch.editBlog(fd).subscribe(
      async res => {
		if(res){
		...
		loading.dismiss();
		localStorage.setItem('editBlog', JSON.stringify(this.blog));
      ...
      async (res) => {
	    ...
	    const loading = await this.loadingCtrl.create({ message: 'Updating ...' });
    await loading.present();
		  this.blog.heading=this.form.value.heading;
	  console.log("edit-data",this.blog);

    this.fetch.editBlog(this.blog).subscribe(
      async res => {
		if(res){
